...
Future<Map> getEnoughOrLack(String recipe_uid, int numOfPlates) async {
  ...
  // numOfPlates 수에 따라 양 배수로 바꾸기.
  // is_countable == false => amount_string을 정수면 int로 바꿔서 곱하고 string으로 바꿔 amount_string에 저장. 분수면 분모, 분자 나눠서 분자*numOfPlates 해서 몫, 나머지로 나눠 다시 string으로 바꿔 amount_string에 저장. ex. 1+1/2큰술
  // is_countable == true => amount_double*numOfPlates 해서 amount_double에 다시 저장.
  ...
  recipeData.forEach((key, value) {
    ...
    // recipeData numOfPlates 수에 따라 양 배수로 바꾸기
    // if (!value['is_countable']) {
    //   print(key);
    //   // 불가산명사
    //   if (value['amount_string'].contains('/')) {
    //     print("분수");
    //     // 분수
    //     fraction = value['amount_string'].split('/');
    //     numerator = fraction[0].toInt();
    //     dominator = fraction[1].toInt();
    //     numerator = numerator * numOfPlates;
    //     if (numerator > dominator) {
    //       // 1 이상
    //       print("1 이상");
    //       value['amount_string'] = (numerator ~/ dominator).toString() + "+" + (numerator % dominator).toString();
    //     } else {
    //       print("1 이하");
    //       // 1 이하
    //       value['amount_string'] = numerator.toString() + '/' + dominator.toString();
    //     }
    //   } else {
    //     // 정수
    //     print("정수");
    //     int temp = int.parse(value['amount_string']);
    //     temp = temp * numOfPlates;
    //     print("들어갔나");
    //     value['amount_string'] = temp.toString();
    //   }
    //   // 가산명사
    //   value['amount_double'] = value['amount_double'].toDouble(); // value['amount_string]이 int로 굳혀져서 오류가 뜨려나?
    //   value['amount_double'] = value['amount_double'] * numOfPlates;
    // }
...
class _MenuDetailPageState extends State<MenuDetailPage> {
  ...
  Widget build(BuildContext context) {
    final args = ModalRoute.of(context)!.settings.arguments
        as Map; // 고정 레시피 recipe_uid, 요리 이름 name, 메뉴 문서 docid, 몇인분 num_of_plates, 메뉴 날짜 menuDateTime
    return MaterialApp(
                      ...
                      children: [
                              ...
                              SizedBox(
                                ...
                                child: TextButton(
                                  ...
                                  child: Row(
                                    ...
                                    children: [
                                      Text(args['num_of_plates'].toString() + "인분", style: inter14Blue),
                                      SizedBox(width: 6),
                                      Transform.rotate(
                        ...
                        Padding(
                          ...
                          child: Row(children: [
                            Expanded(
                                child: TextButton(
                              child: Text("완료하기", style: inter14White),
                              style: TextButton.styleFrom(
                              ...
                              onPressed: () async {
                                if (wholeList != []) {
                                  // 혹시 데이터 불러오기 전에 완료버튼 누르면 실행 안 되도록.
                                  showDialog(
                                    context: context,
                                    builder: (context) {
                                      return AlertDialog(
                                        ...
                                        content: Container(
                                          ...
                                          height: MediaQuery.of(context).size.height / 5, // (체크) 크기 조정하기
                                          child: Column(
                                            children: [
                                              Expanded(
                                                child: Row(
                                                  children: [
                                                    Text("해당 레시피를 ", style: inter18),
                                                    Text("완료", style: interBold18),
                                                    Text("하시겠습니까?", style: inter18)
                                                  ],
                                                ),
                                              ),
                                              Row(
                                                children: [
                                                  Expanded(
                                                    child: TextButton(
                                                        ...
                                                        style: TextButton.styleFrom(
                                                          padding: EdgeInsets.fromLTRB(10, 6, 10, 6),
                                                          side: BorderSide(color: colorPoint, width: 0.5),
                                                          // (체크) figma 줄 두께는 0.5인데 에뮬레이터로 보면 너무 얇아 보임. 실제 기기에서 확인하기
                                                          splashFactory: NoSplash.splashFactory,
                                                          shape: RoundedRectangleBorder(
                                                            borderRadius: BorderRadius.circular(6.0),
                                                          ),
                                                        ),
                                                        child: Text("취소", style: inter17Blue)),
                                                  ),
                                                  SizedBox(width: 10),
                                                  Expanded(
                                                    child: TextButton(
                                                        onPressed: () async {
                                                          await navigatorKey.currentState
                                                              ?.pushNamed('/menu_complete_page', arguments: {
                                                            'wholeList': wholeList,
                                                            'menuDocId': args['docid'], // 메뉴 문서이름
                                                            'menuDateTime': args['menuDateTime'] // 메뉴 날짜
                                                          }); // [{name:가지,is_countable:true,amount_double:2}...]
                                                          setState(() {});
                                                        },
                                                        style: TextButton.styleFrom(
                                                            padding: EdgeInsets.fromLTRB(10, 6, 10, 6),
                                                        ...
                                                        child: Text("확인", style: inter17White)),
                                                  )
                                                ],
                                              )
                                            ],
                                  ...
                                  //         TextButton(
                                  //             onPressed: () async {
                                  //               await navigatorKey.currentState?.pushNamed('/menu_complete_page',
                                  //                   arguments: {
                                  //                     'wholeList': wholeList
                                  //                   }); // [{name:가지,is_countable:true,amount_double:2}...]
                                  //               setState(() {});
                              ...
                              }, // (체크) 완료 버튼 누르면 식재료 자동 차감하는 페이지로 이동.
                            ))
                          ]),
                        ),
                        // 구분선
                        ...
                        FutureBuilder(
                            future: getEnoughOrLack(args['recipe_uid'], args['num_of_plates']),
                            // Map<String, List>
                            ...
                            builder: (context, AsyncSnapshot snapshot) {
                              if (snapshot.hasError) {
                                ...
                                wholeList = List.from(enoughList)..addAll(lackList);
                                return Column(
                                  children: [
                                    // 재료 영역
                                    Padding(
  ...
  Widget howToBox(int sequence, String eachText, String name) {
    return Row(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
